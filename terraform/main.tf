...
resource "aws_cloudwatch_log_group" "app_logs" {
  name              = "/ecs/deploy-stack-vite-example"
  retention_in_days = 14
}

# --- ECR Repository ---
resource "aws_ecr_repository" "app" {
  name                 = "deploy-stack-vite-example-repo"
...
resource "aws_iam_role" "execution_role" {
  name               = "deploy-stack-vite-example-execution-role"
...
resource "aws_iam_role" "task_role" {
  name               = "deploy-stack-vite-example-task-role"
  assume_role_policy = data.aws_iam_policy_document.ecs_trust.json
}

# --- ECS Cluster ---
resource "aws_ecs_cluster" "main" {
  name = "deploy-stack-vite-example-cluster"
}

# --- ECS Task Definition ---
resource "aws_ecs_task_definition" "app" {
  family                   = "deploy-stack-vite-example-task"
  network_mode             = "awsvpc"
  ...
  container_definitions = jsonencode([
    {
      name      = "deploy-stack-vite-example-container"
...
resource "aws_lb" "main" {
  name               = "deploy-stack-vite-example-alb"
...
resource "aws_lb_target_group" "app" {
  name        = "deploy-stack-vite-example-tg"
  port        = 8080
  protocol    = "HTTP"
...
resource "aws_ecs_service" "app" {
  name            = "deploy-stack-vite-example-service"
  ...
  load_balancer {
    target_group_arn = aws_lb_target_group.app.arn
    container_name   = "deploy-stack-vite-example-container"
    container_port   = 8080
  }
...
resource "aws_iam_role_policy" "secrets_policy" {
  name   = "deploy-stack-vite-example-secrets-policy"
